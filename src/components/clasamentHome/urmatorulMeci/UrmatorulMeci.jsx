...
const UrmatorulMeci = () => {
  ...
  useEffect(() => {
    async function fetchMeciViitor() {
      try {
        const response = await fetch('http://localhost:5050/api/meci/meciviitor');
        if (!response.ok) {
        ...
        if (data && data.datameci) {
            ...
            const formattedDateTime = new Date(dateTime.toLocaleString('en-US', { timeZone: 'Europe/Bucharest' }));
            const formattedDate = formatRomanianDate(formattedDateTime);
            const formattedTime = formatRomanianTime(formattedDateTime);
            const hour = formattedDateTime.getHours(); // Ora este extrasă aici
